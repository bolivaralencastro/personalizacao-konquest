...
const SessaoPersonalizadaApp = (function() {
    ...
    function handleSessionsListClickEvt(e){
        ...
        if(e.target.closest('.session-card-status[data-action="open-schedule-popover"]')){
            // Lógica de toggle para o popover de agendamento
            const popover = DOM.schedulePopover;
            if (popover && (popover.style.display === 'block' || popover.style.display === 'flex')) {
                // Se já estiver aberto, fechar
                closeSchedulePopover();
            } else {
                // Se estiver fechado, fechar outros popovers e abrir
    ...
    function init() {
        ...
        // Adicionar listener para o botão Cancelar da modal de exclusão
        const cancelDeleteBtn = DOM.confirmDeleteModal.querySelector('.cancel-btn');
        if (cancelDeleteBtn) {
